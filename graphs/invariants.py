...
def independence_number(G: nx.Graph) -> int:
    ...
    if n <= 20:
        ...
        try:
            cliques = list(nx.find_cliques(comp))
            return max(len(c) for c in cliques)
        except Exception:
            pass

    # Fallback: approximation via networkx
    try:
        ind_set = nx.approximation.maximum_independent_set(G)
        return len(ind_set)
    except Exception:
        return n - G.number_of_edges()  # very rough lower bound
...
def domination_number(G: nx.Graph) -> int:
    ...
    # Greedy: always pick vertex that dominates the most un-dominated
    dominated: set = set()
    chosen = 0
    remaining = set(nodes)
