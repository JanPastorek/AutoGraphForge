...
class GraphOfThoughtFormalizer:
    ...
    def _heuristic_lean_statement(self, conjecture: Conjecture) -> Optional[str]:
        """
        Produce a best-effort Lean 4 statement without an LLM call,
        for TxGraffiti-style inequalities with supported invariants.
        """
        ineq = conjecture.inequality
        if ineq is None:
            return None
        ...
        a_lean = inv_map.get(ineq.inv_a)
        b_lean = inv_map.get(ineq.inv_b)
        if a_lean is None or b_lean is None:
            return None

        coeff_str = f"{int(ineq.coeff_b)} * " if ineq.coeff_b != 1.0 else ""
        offset_str = f" + {int(ineq.offset)}" if ineq.offset != 0.0 else ""
        theorem_name = (
            f"conj_{ineq.inv_a}_le_{int(ineq.coeff_b)}_mul_{ineq.inv_b}"
            + (f"_plus_{int(ineq.offset)}" if ineq.offset else "")
        )

        return textwrap.dedent(f"""
        -- Conjecture: {conjecture.statement}
        -- Generated by heuristic autoformalization (no LLM)
        theorem {theorem_name}
            (V : Type*) [Fintype V] [DecidableEq V]
            (G : SimpleGraph V) [DecidableRel G.Adj] :
            {a_lean} ≤ {coeff_str}{b_lean}{offset_str} := by
          sorry
        """).strip()
