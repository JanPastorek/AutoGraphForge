...
class Conjecture(SageObject):  # Based on GraphExpression from IndependenceNumberProject
    ...
    def evaluate(self, g, returnBoundValue=False):
        stack = []
        if returnBoundValue:
            assert self.stack[-1][0] in {operator.le, operator.lt,
                                         operator.ge, operator.gt}, "Conjecture is not a bound"
...
def _makeConjecture(inputList, variable, invariantsDict):
    ...
    binaryOperators = {'+': operator.add, '*': operator.mul,
                       '-': operator.sub, '/': operator.truediv, '^': operator.pow}
    comparators = {'<': operator.lt, '<=': operator.le,
    ...
    for op in inputList:
        if op in invariantsDict:
            ...
            expressionStack.append(
                sage.symbolic.function_factory.function(op)(variable))
            ...
            raise ValueError(
                "Error while reading output from expressions. Unknown element: {}".format(op))

    return Conjecture(operatorStack, expressionStack.pop(), (inputList, variable, invariantsDict))


def handleSpecialOperators(stack, op):
    if op == '-1':
...
def conjecture(objects, invariants, mainInvariant, variableName='x', time=5,
    ...
    if not precomputed:
        ...
        assert len(
            precomputed) == 3, 'The length of the precomputed tuple should be 3.'
        precomputed, object_key, invariant_key = precomputed
    else:
        def object_key(x): return x
        def invariant_key(x): return x

    assert 0 <= mainInvariant < len(
        invariants), 'Illegal value for mainInvariant'
    ...
    stdin.write('{} {} {}\n'.format(
        len(objects), len(invariants), mainInvariant + 1))
    ...
    if theory is not None:
        ...
        for o in objects:
            if upperBound:
                try:
                    stdin.write('{}\n'.format(
                        min(float(get_value(t, o)) for t in theory)))
                except:
                    stdin.write('NaN\n')
            else:
                try:
                    stdin.write('{}\n'.format(
                        max(float(get_value(t, o)) for t in theory)))
                except:
                    stdin.write('NaN\n')
        if verbose:
    ...
    for o in objects:
        for invariant in names:
            try:
                stdin.write('{}\n'.format(
                    float(get_value(invariantsDict[invariant], o))))
            except:
                stdin.write('NaN\n')
    ...
    for l in out:
        ...
        if op:
            ...
            conjectures.append(_makeConjecture(
                inputList, variable, invariantsDict))
            inputList = []
...
def propertyBasedConjecture(objects, properties, mainProperty, time=5, debug=False,
    ...
    if len(properties) < 2 or len(objects) == 0:
        return
    if not theory:
        theory = None
    if not precomputed:
    ...
    assert 0 <= mainProperty < len(
        properties), 'Illegal value for mainProperty'

    operatorDict = {'~': 'U 0', '&': 'C 0',
                    '|': 'C 1', '^': 'C 2', '->': 'N 0'}
    ...
    stdin.write('{} {} {}\n'.format(
        len(objects), len(properties), mainProperty + 1))
    ...
    if theory is not None:
        ...
        for o in objects:
            if sufficient:
                try:
                    stdin.write('{}\n'.format(
                        max((1 if bool(get_value(t, o)) else 0) for t in theory)))
                except:
                    stdin.write('-1\n')
            else:
                try:
                    stdin.write('{}\n'.format(
                        min((1 if bool(get_value(t, o)) else 0) for t in theory)))
                except:
                    stdin.write('-1\n')
        if verbose:
            print("Finished writing theory to expressions")
    ...
    for o in objects:
        for property in names:
            try:
                stdin.write('{}\n'.format(
                    1 if bool(get_value(propertiesDict[property], o)) else 0))
            except:
                stdin.write('-1\n')
    ...
    for l in out:
        ...
        if op:
            ...
            conjectures.append(_makePropertyBasedConjecture(
                inputList, propertiesDict))
            inputList = []
