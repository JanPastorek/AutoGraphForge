...
Computer-assistance in this area can be roughly classified into three areas based on the research stage it assists: (i) \emph{conjecturing}, which generates new statements that are hypothetised to be true based on a finite database (of graphs); (ii) \emph{refuting}, which searches for counterexamples to conjectures; and (iii) \emph{formalizing and proving}, which translates conjectures into a formal language and attempts to prove them using automated theorem provers.
...
A practitioner who reimplements such a system quickly meets four possible failures.
...
Lastly, there is a growing body of work on autoformalization and automated theorem proving in graph theory, see for instance \cite{gonthier2013formal,blanchette2020hammer,li2021graph,li2022graph,li2023graph}. However, altough we recently found in \cite{davila2026graffiti3} a certain hints in this direction, to the best of our knowledge, there is no existing system that integrates automated conjecturing, refuting, formalizing, and proving in a closed loop for graph theory. 

In this paper, we describe and report on the development of a computational pipeline that aims to combine (i)-(iii) in a closed loop. The system \emph{autograph} iteratively refines conjectures by  generating and filtering conjectures and trying to refute them in a loop, and formalizes the surviving candidates into a proof assistant for automated theorem proving.

Our current contributions are:

\begin{enumerate}
  \item We build the first version of the computational pipeline \emph{autograph} and run it on a high-performance computing cluster. 
...
\item A validated counterexample-search engine
  (\S\ref{sec:engine}) that includes among others recent state-of-the-art methods such deep cross-entropy reinforcement learning or Monte Carlo methods. To validate it we reproduced the refutation of a number of conjectures and exhaustively verified a
...
We note that our system is not build from scratch but is built upon existing packages, frameworks and methodologies present in the state-of-the-art literature on each of the three areas of conjecturing, refuting, and formalizing and proving. In particular, the conjecture generation are built on top of TxGraffiti and Conjecturing; the counterexample search engine is built upon recent state-of-the-art methods such deep cross-entropy reinforcement learning or Monte Carlo methods; and the formalization and proving layer is built upon recent autoformalization methods and automated theorem provers.
...
experiments, and help with the draft of the manuscript. All computational results were verified against authoritative data source -- House of Graphs and exact recomputation; the
