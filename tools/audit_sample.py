...
def main() -> int:
    ...
    ap.add_argument("--normalise", action="store_true",
                    help="collapse restatements (same canonical body and "
                         "hypothesis, different surface form). Off by default so "
                         "the published listing and Table 2 stay reproducible.")
    ...
    # Two maps, because they mean different things. `published` is the
    # behaviour the released figures were computed with: hypothesis
    # subsumption only. `full` additionally collapses *restatements* — same
    # canonical body, same hypothesis, different surface form, e.g.
    # `b <= r + 1` and `b - 1 <= r`, or `t <= 3d` and `(1/3)t <= d`. The
    # difference between the two is exactly the set of restatements, and it is
    # kept separate so the published listing stays reproducible.
    subsumed_published = cl.find_subsumed(survivors, collapse_duplicates=False)
    subsumed_full = cl.find_subsumed(survivors)
    restatements = set(subsumed_full) - set(subsumed_published)
    subsumed = subsumed_full if args.normalise else subsumed_published
    records = []
    for i, s in enumerate(survivors):
        ...
        records.append({
            ...
            "restatement_of": (survivors[subsumed_full[i]].statement
                               if i in restatements else None),
    ...
    if args.listing:
        ...
        with open(args.listing, "w") as fh:
            shown = [r for r in records if not r.get("restatement_of")]
            dropped = len(records) - len(shown)
            fh.write("# AutoGraphForge survivors, ranked by touch number\n")
            if args.normalise:
                fh.write("# NORMALISED: restatements collapsed.\n"
                         "# A conjecture and its algebraic rearrangement are one\n"
                         "# claim, not two: `b <= r + 1` and `b - 1 <= r` differ by\n"
                         "# moving a constant across the relation, `t <= 3d` and\n"
                         "# `(1/3)t <= d` by clearing a denominator. Both normalise\n"
                         "# to a single canonical body. They were reported twice\n"
                         "# because they are generated by different partitions --\n"
                         "# one is a bound on t, the other a bound on d -- so no\n"
                         "# single generator ever sees the pair.\n")
                fh.write(f"# {dropped} restatement(s) removed here; each is listed\n"
                         "# in the trailing section with the form that was kept.\n")
            ...
            fh.write(f"## Ranked by touch ({len(shown)})\n\n")
            for r in shown:
                fh.write(f"[{r['touch_generation']:6d}] {r['statement']}"
                         f"    -- {r['bucket']}\n")
            if args.normalise and dropped:
                fh.write(f"\n\n## Collapsed restatements ({dropped})\n")
                fh.write("# Each line: the form dropped, then the form kept.\n\n")
                for r in records:
                    if r.get("restatement_of"):
                        fh.write(f"[{r['touch_generation']:6d}] {r['statement']}\n"
                                 f"         == {r['restatement_of']}\n")
            ...
            for stmt in sorted(rest):
                fh.write(f"[     -] {stmt}\n")
        print(f"\nwrote {args.listing}  ({len(shown)} ranked + "
              f"{len(conjectures) - len(records)} unranked"
              + (f", {dropped} restatements collapsed)" if dropped else ")"))
